...
### 5.1 — Kill the estimated/fabricated baseline — **BLOCKER**
...
### 5.2 — Reframe the 38–48% number honestly
...
### 5.4 — Separate latency and throughput experiments — **throughput MISSING**
...
### 5.5 — In-band parity gate on every timing run — **MISSING**
...
### 5.7 — Two cheap, high-value measurements
...
### 5.8 — The privacy-cost-in-modulus-bits result
...
### 6.1 — Strip build artifacts from git — **MISSING hygiene**
- Remove committed `build/` and `vendor_server/build/` (~50 MB of objects/binaries and `/home/raghavp/BTP` absolute paths). Artifact evaluators flag this immediately.
...
### 6.3 — Add what's missing for artifact evaluation
- Add a `LICENSE`.
- Fix `README.msd` → `README.md` (typo extension); remove the dangling reference to a `bank_client/frontend` FastAPI app that doesn't exist.
- **Scripted figure regeneration:** a single `make reproduce` / one-command path from raw results JSON to every figure. `generate_research_artifacts.py` currently emits CSVs but no figures and depends on a running server + uncommitted artifacts.
- **Script-enforced key-gen pipeline** (currently documented as prose ordering).
- Fixed seeds are already present (`random_state=42` in `train_xgboost.py`, `linearize.py`) — keep and document.

### 6.4 — Fix the dataset/accuracy methodology defects
- **File:** `compiler/train_xgboost.py` — winsorization uses **test-set** quantiles (leakage); features truncated to first 256 polynomial terms arbitrarily. Re-do with train-only statistics; justify or revisit the 256 cap. The 0.979 AUC will move; report the honest number.
- **File:** `compiler/linearize.py` — does **not** linearize XGBoost; it trains an independent `sklearn.LogisticRegression`. Stop calling it a "compiler"/"linearization." Report the XGBoost-vs-LR AUC gap plainly as the cost of using a linear model.
